...
<div class="modern-card">
    ...
    @else
                ...
                <tbody>
                    ...
                    @php
                        ...
                        if ($mov->source_type === \App\Models\Sale::class) {
                            ...
                            $sourceUrl  = route('admin.v3.vehicles.index');
                            $sourceIcon = 'bi-car-front';
                            $sourceTitle = 'Ver Viaturas';
                        }
                    @endphp
        ...
        {{-- Mobile: resumo do período + cards --}}
        <div class="d-lg-none">
            @php
                $periodGrossMobile = $movements->sum(fn($m) => $m->isIncome() ? ($m->amount_gross ?? $m->amount) : -($m->amount_gross ?? $m->amount));
                $periodNetMobile   = $movements->sum(fn($m) => $m->isIncome() ? ($m->amount_net ?? $m->amount) : -($m->amount_net ?? $m->amount));
            @endphp
            <div class="movement-summary-mobile p-3">
                <div class="movement-summary-row">
                    <span>Total bruto do período</span>
                    <strong class="{{ $periodGrossMobile >= 0 ? 'text-success' : 'text-danger' }}">
                        {{ $periodGrossMobile >= 0 ? '+' : '' }}€ {{ number_format($periodGrossMobile, 2, ',', '.') }}
                    </strong>
                </div>
                <div class="movement-summary-row">
                    <span>Total líquido do período</span>
                    <strong class="{{ $periodNetMobile >= 0 ? 'text-success' : 'text-danger' }}">
                        {{ $periodNetMobile >= 0 ? '+' : '' }}€ {{ number_format($periodNetMobile, 2, ',', '.') }}
                    </strong>
                </div>
                <div class="movement-summary-row">
                    <span>Saldo</span>
                    <strong class="text-primary">€ {{ number_format($currentSaldo, 2, ',', '.') }}</strong>
                </div>
                <div class="movement-summary-row">
                    <span>Saldo real</span>
                    <strong class="text-success">€ {{ number_format($currentSaldoReal, 2, ',', '.') }}</strong>
                </div>
            </div>
            ...
            <div class="p-3 pt-0">
                ...
                @php
                    ...
                    $movActions = $isAutoGenerated
                        ? array_filter([
                            $sourceUrl ? ['href' => $sourceUrl, 'icon' => $sourceIcon, 'label' => $sourceTitle, 'color' => 'secondary'] : null,
                        ])
                        : [
                            ['href' => route('admin.v2.movements.edit', $mov->id), 'icon' => 'bi-pencil', 'label' => 'Editar', 'color' => 'primary'],
                            ['href' => route('admin.v2.movements.destroy', $mov->id), 'icon' => 'bi-trash', 'label' => 'Eliminar', 'color' => 'danger', 'method' => 'DELETE', 'confirm' => 'Eliminar este movimento?'],
                        ];
                @endphp
                @include('components.admin.item-card', [
                    'title' => $mov->title . ($isAutoGenerated ? ' (Auto)' : ''),
                    'subtitle' => $mov->v3Vehicle
                        ? $mov->v3Vehicle->brand . ' ' . $mov->v3Vehicle->model . ' (' . $mov->v3Vehicle->reference . ')'
                        : ($mov->client->name ?? null),
                    'badges' => [
                        ['text' => $mov->movement_type_label, 'color' => $typeColor],
                        ['text' => $mov->category_label, 'color' => 'secondary'],
                        ['text' => $mov->status_label, 'color' => $statusColor],
                    ],
                    'meta' => [
                        ['icon' => 'bi-calendar3', 'text' => $mov->expense_date ? $mov->expense_date->format('d/m/Y') : '—'],
                        ['icon' => 'bi-cash', 'text' => ($isIncome ? '+' : '-') . '€ ' . number_format($mov->amount_net ?? $mov->amount, 2, ',', '.') . ' líquido'],
                    ],
                    'actions' => $movActions,
                ])
                @endforeach
        ...
        @include('components.admin.pagination-footer', ['items' => $movements, 'label' => 'movimentos'])
    @endif
</div>

@push('styles')
<style>
    .movement-summary-mobile {
        display: flex;
