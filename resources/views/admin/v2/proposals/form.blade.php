    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            ...
            <div class="modern-card" id="shareCard">
                ...
                {{-- Instrução rápida para o comercial --}}
                <p class="small text-muted mb-3">
                    Copie o link abaixo e envie ao cliente. O cliente pode ver a cotação e aceitá-la directamente.
                </p>

                {{-- Link copiável --}}
                @php $shareUrl = route('proposals.detail', $proposal->proposal_code); @endphp
                <div class="input-group input-group-sm mb-3">
                    <input type="text" id="shareLink" class="form-control font-monospace"
                ...
                @php
                    $waMessage = urlencode(
                        ...
                        . "Preparámos uma cotação personalizada para o seu " . $proposal->brand . " " . $proposal->model . ".\n\n"
                        . "Pode ver todos os detalhes e aceitar aqui:\n"
                        . $shareUrl . "\n\n"
                        . "Qualquer dúvida estamos à disposição. 🚗"
                    );
                    // Remove espaços e caracteres não numéricos do telefone, adiciona indicativo PT se necessário
                    $rawPhone  = preg_replace('/\D/', '', $proposal->client?->phone ?? '');
                    $waPhone   = $rawPhone ? (str_starts_with($rawPhone, '351') ? $rawPhone : '351' . $rawPhone) : '';
                ...
                <div class="d-flex gap-2 flex-wrap">
                    ...
                    <a href="{{ $waUrl }}" target="_blank" rel="noopener"
                       class="btn btn-sm btn-outline-success flex-fill" style="color:#25D366;border-color:#25D366">
                    ...
                    <a href="mailto:{{ $proposal->client->email }}?subject={{ urlencode('A sua cotação Izzycar — ' . $proposal->brand . ' ' . $proposal->model) }}&body={{ urlencode("Olá " . $proposal->client->name . ",\n\nSeguimos em anexo o link da sua cotação personalizada:\n\n" . $shareUrl . "\n\nQualquer dúvida estamos à disposição.\n\nIzzycar") }}"
                ...
                {{-- Badge de estado de aceitação --}}
                @php $isAccepted = \App\Models\ConvertedProposal::where('proposal_id', $proposal->id)->exists(); @endphp
                @if($isAccepted)
                <div class="alert alert-success mt-3 mb-0 py-2 small">
                    <i class="bi bi-check-circle-fill me-1"></i>
                    <strong>Cotação aceite pelo cliente.</strong>
                </div>
                @else
                <div class="alert alert-light mt-3 mb-0 py-2 small text-muted">
