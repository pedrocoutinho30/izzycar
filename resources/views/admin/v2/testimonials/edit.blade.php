                        ...
                        <div class="col-md-6">
                            ...
                            <select name="origin" class="form-select @error('origin') is-invalid @enderror" required>
                                @foreach($origins as $key => $label)
                                    <option value="{{ $key }}" {{ old('origin', $testimonial->origin) === $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                ...
                <div class="modern-card-body">
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" name="published" id="published" value="1"
                               {{ old('published', $testimonial->published) ? 'checked' : '' }}>
                        <label class="form-check-label fw-semibold" for="published">Publicado</label>
                        <div class="form-text">Testemunhos publicados ficam visíveis no site.</div>
                    </div>
                    <hr>
                    <div class="text-muted small">
                        <i class="bi bi-clock me-1"></i> Criado em {{ $testimonial->created_at->format('d/m/Y H:i') }}
                    </div>
                </div>
            </div>
